...
func RoActivateInstance(clsid string) (ins *iinspectable.Inspectable, err error) {
...
func RoGetActivationFactory(clsid string, iid *com.GUID) (ins *iinspectable.Inspectable, err error) {
	hClsid, err := NewHString(clsid)
	if err != nil {
		return nil, err
	}
